...
#if defined(NDEBUG)
# error "CryptoSourceCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x10ed6ad3a180959b02d86275190fec6d82d56b1b48855c42022a7b30c63f6555");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // CryptoSourceCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "CryptoSourceCoin Signed Message:\n";
...
{
    // CryptoSourceCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1680000); // CryptoSourceCoin: 1680k blocks in ~8 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // CryptoSourceCoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // CryptoSourceCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // CryptoSourceCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // CryptoSourceCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2680c87b6f5b569726f2ee29641b124ef81fd5deb7cec1447d0fa776545e1b47");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is CryptoSourceCoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04261518d3cac6a58d5e017b14bd0905ea27212822909bd903912396db4543476f758016be07e140ed6f7f5db46eb743d7b61eb1cbdeca08a07082eaf706148089") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1525453762;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086242591;

        if (fTestNet)
        {
            block.nTime    = 1525453100;
            block.nNonce   = 386886563;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xb9d7118f6c704288e2bbc277fd136fe9e713794bb9fb7ed32ecd6f48aa36ab74"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // CryptoSourceCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CryptoSourceCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CryptoSourceCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CryptoSourceCoinMiner(CWallet *pwallet)
{
    printf("CryptoSourceCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cryptosourcecoin-miner");
    ...
    try { loop {
        ...
        printf("Running CryptoSourceCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CryptoSourceCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CryptoSourceCoinMiner, pwallet));
}
